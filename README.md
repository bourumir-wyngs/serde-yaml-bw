...
use serde::Serialize;
use std::sync::Arc;
use serde_yaml_bw::{ArcAnchor, SerializerOptions, to_string_with_options};

#[derive(Serialize)]
struct Item { id: u8 }

let shared = ArcAnchor(Arc::new(Item { id: 7 }));

let mut options = SerializerOptions::default();
options.set_anchor_name_fn(|index| format!("node_{index}"));

let yaml = to_string_with_options(&vec![shared.clone(), shared], &options)?;
